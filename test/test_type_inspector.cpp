...
std::vector<TypeInfo> Setup(const char* source, bool* error);
std::vector<TypeInfo>
Setup(const char* source, const std::vector<std::string>& arguments, bool* error);
...
TEST_CASE("Type Inspector")
{
  SECTION("Can parse one type with no fields from a source file")
  {
    ...
    auto types = Setup(source, nullptr);
...
std::vector<TypeInfo> Setup(const char* source, bool* error)
{
  ...
  return Setup(source, empty, error);
}
