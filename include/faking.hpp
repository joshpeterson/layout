...
template <typename T> class FakeHelper
{
public:
  FakeHelper(T* fake, T testDouble) : fake_(fake) { *fake_ = testDouble; }

  ~FakeHelper() { *fake_ = nullptr; }

private:
  T* fake_;
};

template <typename T> FakeHelper<T> Fake(T* fake, T testDouble)
{
...
#define DEFINE_FAKE(name) name##Function name##Fake_ = nullptr;
...
#define FAKEABLE(returnType, name, arguments) returnType name arguments;
